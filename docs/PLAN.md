...
| Distribution | Official Hermes `v2026.9.7` source archive and container recipe are pinned and locally build-verified; reproducible multi-target Host builds and a one-line platform-detecting installer exist and are contract-tested; the release manifest remains a fixture because the pinned Hermes image is not yet published | Publish the pinned Hermes image, generate the manifest from real assets, and record the download |
