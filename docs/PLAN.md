...
1. **Foundation:** prove live replacement/rollback on a real deployment, pinned-runtime lost-stream recovery, and cross-machine external Runs. Milestone 1 stays open until those real checks pass.
...
The conversation nucleus now has test-backed profile and endpoint checks, required artifact and process identity fields, preference projection, deadline rejection, durable session reservation, forbidden-event invalidation, and bounded restart reconciliation. A second exact certification check after projection rejects a changed reported process before dispatch; the request itself is not yet bound to that process. Independent security review of restart reconciliation passed. Pinned Hermes `v2026.9.7` fails open when a `pre_tool_call` plugin callback raises, so the plugin cannot be the zero-tool chat authority. A dedicated patched runtime passes disposable synthetic negative probes for forced tool calls, room route admission, context/memory disclosure, session replay, and auxiliary title calls. The conversation gate remains **open** until artifact/process/configuration/endpoint binding supports a production Host certifier, and the two-client, Host-restart, and Hermes-restart journeys pass on intended deployments. No product-surface milestone is complete merely because package tests pass.
...
Owner-authored canonical memory now has save/list/delete control commands and bounded future-context projection. This does not establish production Hermes chat or model-proposed memory. The next gate is a deployed chat certificate bound to artifact, process, configuration, and endpoint, followed by a real two-client send/restart/delete-memory journey.

Conversation execution now emits redacted structured stage logs to the Host's stderr stream. These show the certification stop or, when a runtime is certified, projection sizes, Hermes acceptance, evidence receipt, and durable terminal outcome without logging private content.

## Current execution lock

**Milestone 1 foundation closure and the bounded Milestone 2 conversation nucleus are the active implementation lanes.** The durable Go Space authority, encrypted store, authenticated local control boundary, Host-to-Hermes Runs execution, dual-topology public setup, and durable doctor/service paths exist. The combined gate passed twice clean on Azure Ubuntu 22.04 amd64 and survived a real reboot. External setup against an independently supervised, pinned Hermes runtime passed authenticated mutual-TLS adoption, Host-only lifecycle control, doctor, canonical-state preservation, and a real reboot on that same VPS; the automated external journey also passed twice clean with an isolated compatibility endpoint. The release manifest now resolves to real integrity-pinned artifacts, and exact deny and cancel are proven against the pinned Hermes runtime by a native macOS Host journey. Live deployment update/rollback has not been exercised on a real machine, container-image generations are unsupported, pinned-runtime lost-stream recovery remains open, and real cross-machine external Runs behavior remains unproven. M1 remains active.

The owner authorized the bounded conversation nucleus on 2026-09-15 after a real-model manual journey showed that the generic Hermes API caller selected web, files, terminal, and memory tools without Lumen-owned identity, context, or policy. Host-owned conversation state and recovery exist on this branch, but production chat dispatch remains blocked until deployment certification and real continuity journeys pass. Node transport, web UI, messaging continuity, voice, and device capabilities remain out of scope.

`lumen update` and `lumen rollback` advance or restore one integrity-pinned artifact generation under the immutable setup binding; see [the foundation gate](./PHASE-2-HOST-HERMES.md). Platform journeys stay explicit: `milestone1-linux-check` and `milestone1-external-check` cover the Docker topologies, while `milestone1-macos-check` runs the Host natively on darwin because a host-shared bind mount cannot provide the control socket's enforceable `0600` mode. The setup branch is merged; product-surface implementation still depends on the remaining M1 evidence. Physical Android, Mac, or iPhone completion is not a prerequisite unless it proves the active milestone's contract.
...
| Setup and public lifecycle | Combined and external Docker journeys pass; real Azure reboot, doctor, state preservation, and Host-only external lifecycle are recorded; a public `lumen update`/`lumen rollback` generation path has automated tests | Prove live replacement/rollback on a real machine |
| Distribution | Release `v0.1.0-m1` publishes five reproducible Host artifacts and a one-line installer; `deploy/manifest-v1.json` is integrity-pinned and verified end to end | Run the real container update/rollback journey and make the Hermes package anonymously pullable |
