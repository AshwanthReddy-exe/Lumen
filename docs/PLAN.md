...
The existing conversation-nucleus branch has six Important review findings. This branch has test-backed changes for certificate binding (I1), preference projection (I3), deadline rejection (I5), durable session reservation before certification I/O (I4), and forbidden-event invalidation (I2). The I2 change passed independent review and encrypted-store reopen testing. Replay (I6) has partial hardening. The conversation gate remains **open** until queued/dispatched chat survives restart with verified completion and truthful replay (I6), and the real Host restart path exercises the reservation. Then wire a real certifier and run the two-client, Host-restart, Hermes-restart, and zero-tool journeys. No product-surface milestone is complete merely because package tests pass.
