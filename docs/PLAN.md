...
The conversation nucleus now has test-backed certificate binding, preference projection, deadline rejection, durable session reservation, forbidden-event invalidation, and bounded restart reconciliation. Independent security review of restart reconciliation passed. Pinned Hermes `v2026.9.7` fails open when a `pre_tool_call` plugin callback raises, so the plugin cannot be the zero-tool chat authority. The conversation gate remains **open** until a dedicated zero-tool runtime profile and real negative tool/memory probes support a production certifier, and the two-client, Host-restart, and Hermes-restart journeys pass on intended deployments. No product-surface milestone is complete merely because package tests pass.

A [disposable chat-profile preflight](./evidence/HERMES-CHAT-PREFLIGHT-2026-09-25.md) now checks resolved toolsets and constructed-agent memory against pinned Hermes. Its live synthetic Runs probe is incomplete evidence: process binding, session/context isolation, and the room-policy override route remain unqualified. The Host continues to deny production conversation dispatch.
...
| Conversations and memory | Host-owned conversation state, bounded run evidence, and restart reconciliation are implemented; production chat remains blocked without a qualified zero-tool Hermes profile | Qualify a dedicated pinned runtime with negative tool/memory probes, then wire certification and prove two-client continuity |
