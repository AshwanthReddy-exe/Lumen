...
The existing conversation-nucleus branch has six Important review findings. This branch has test-backed changes for certificate binding (I1), preference projection (I3), deadline rejection (I5), and durable session reservation before certification I/O (I4). Event rejection (I2) and replay (I6) have partial hardening. The conversation gate remains **open** until unexpected runtime events also invalidate certification with redacted evidence (I2), session reservation is exercised against the real encrypted store and restart path (I4), and queued/dispatched chat survives restart with verified completion and truthful replay (I6). Then wire a real certifier and run the two-client, Host-restart, Hermes-restart, and zero-tool journeys. No product-surface milestone is complete merely because package tests pass.
