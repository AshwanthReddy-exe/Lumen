...
| D-049 | Split Hermes integration into an inbound `HermesRuntimeAdapter v1` and a separate narrow outbound Lumen Hermes plugin/broker. | The adapter handles authenticated capability/Run evidence for one runtime; the plugin exposes only typed capability intents and redacted inventory. Neither can grant, select a target, approve itself, or persist canonical Space state. [Boundary](./research/HERMES-LUMEN-INTEGRATION.md) |
| D-050 | Keep a Host-owned feature registry and one active Hermes runtime per Host. | The full Hermes surface is descriptive until certified and mapped to immutable profiles and task capabilities. Runtime replacement or rebinding is explicit; sessions and profiles never become Space authority. [Research](./research/HERMES-LUMEN-INTEGRATION.md) |
...
| D-053 | Deliver one Lumen agent across devices through a Host-owned Space, with Hermes as the preferred runtime and Future OS as a product and implementation reference. | Every surface reads and writes the same canonical conversation and task records. Future OS interface, remote-control and release patterns may be adapted after license and security review; its agent sessions, permissive defaults and process-local approvals never replace Lumen authority. Runtime substitution remains possible behind the existing adapter. |
