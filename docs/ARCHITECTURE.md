...
On pinned Hermes `v2026.9.7`, plugin `pre_tool_call` callbacks fail open on exceptions. A plugin veto therefore cannot establish zero-tool isolation. Production conversation certification must be based on a dedicated, qualified runtime profile with pinned artifact/configuration evidence and real negative tool and memory probes. Until then the Host rejects chat before disclosing conversation content; a plugin hook may add defense in depth.
