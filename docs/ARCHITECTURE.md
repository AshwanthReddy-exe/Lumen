...
Milestone 1 has two explicit setup topologies. In **combined**, the Docker Compose reference deployment owns and separately supervises the Lumen Host and Hermes artifacts, configuration, and services. In **external**, Lumen owns only the local Host artifact and service and adopts an existing independently managed Hermes endpoint; it never installs, configures, restarts, or inspects the remote Hermes machine. The topology changes artifact and lifecycle ownership, not Space authority or task behavior. External service commands control only the Host, while Hermes availability is a live observation used for `ready` or `degraded` status.
...
On pinned Hermes `v2026.9.7`, plugin `pre_tool_call` callbacks fail open on exceptions. A plugin veto therefore cannot establish zero-tool isolation. The dedicated chat process uses a source-pinned patch that rejects model tool calls before execution, rejects room dispatch, and skips Hermes context and memory; its configuration also disables auxiliary title requests. The Host has a separate conversation-runtime slot and cannot fall back to its general-purpose Hermes adapter. It rechecks the exact certificate and endpoint after private-context projection and rejects changed reported binding before dispatch. This does not yet bind the subsequent request connection to the certified process. Disposable source and container negative probes cover those paths. Production conversation certification still requires binding the deployed artifact, process, configuration, and endpoint to the Host and proving recovery. Until then the Host rejects chat before disclosing conversation content; a plugin hook may add defense in depth.
...
The Host owns the feature registry. Hermes discovery contributes untrusted metadata; Lumen records the feature's version, provenance, required permissions and credentials, data flow, risk, compatibility evidence, health, fallback, rollback, and kill switch, then exposes it only through an immutable, verifiable runtime profile and task capability. MCP servers, skills, and tools remain pinned and reviewed executable dependencies. A Hermes remote-execution backend is deployment configuration for that worker, not a node transport or authorization shortcut.

V1 binds each Host to one active, compatibility-certified Hermes runtime endpoint. Profiles and workers are bounded execution configuration inside that runtime, not additional authorities; changing the endpoint, profile binding, or topology requires explicit revalidation and rejects stale identity or epoch evidence. Hermes session IDs map to replaceable Host task or conversation records and never own Space history.
