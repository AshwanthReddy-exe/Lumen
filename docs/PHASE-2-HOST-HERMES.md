...
The combined Docker gate passed twice clean on Azure Ubuntu 22.04 amd64 and survived an actual reboot with stable service identities and canonical state. The external path passed public mutual-TLS adoption against independently supervised pinned Hermes, Host-only lifecycle control, doctor, state preservation, and another actual reboot on the same VPS. Its automated isolated-endpoint journey passed twice clean. These results qualify the tested lifecycle boundaries, but they do not create a distributable release, prove live artifact replacement, or establish cross-machine external Runs behavior.

| Milestone 1 deliverable | Baseline status | Evidence and remaining gap |
| --- | --- | --- |
| 1. Connect the public CLI to planning, journal, artifacts, configuration, Host initialization, supervision, and validation | Implemented and Linux-verified | Combined and external public journeys preserve create-once identity and durable bindings. External setup owns exactly the Host and adopts Hermes before Host mutation. |
| 2. Truthful `lumen setup`, `lumen doctor`, and service lifecycle behavior | Implemented and Docker-verified | Combined and external Azure proofs returned `ready`; reboot recovery preserved service identity and canonical state. Stopping/restarting external Host left independently supervised Hermes unchanged. Launchd/runit boot enablement remains fail-closed until an authoritative query is implemented. |
| 3. Reproducible integrity-pinned Lumen and Hermes distribution | Implemented | Hermes `v2026.9.7` resolves to peeled commit `2237be355906fbe6065ce1815711eee52b2d646e`; the archive SHA-256 is pinned as `907c2a72db1c5dd637ea8eeae97f4cb5b32cef615c17258f6b190924ec5bf688`. The Hermes runtime is a public multi-architecture image `ghcr.io/ashwanthreddy-exe/lumen-hermes@sha256:314d1e03ec846a6b0b0d81381ceca84b59ad4acd0101ae301f9879c5428ac103` for `linux/amd64` and `linux/arm64`. Release `v0.1.0-m1` publishes `linux/amd64`, `linux/arm64`, `darwin/amd64`, `darwin/arm64`, and `android/arm64` Host artifacts built twice per target, plus `install.sh` and `manifest-v1.json`. `deploy/manifest-v1.json` is no longer a fixture. Verified end to end: the manifest resolves through `releases/latest/download`, the published `lumen-host-linux-amd64` hashes to the exact pinned digest `a50a28e201f1a4e6dda26aaa9fef1b80c84e03220e58103fdfe0bba884708f93` (7,721,120 bytes), and the published installer downloaded and verified a real artifact. Cross-run reproducibility required disabling Go's default VCS stamping, which had produced artifacts that did not reproduce from a clean checkout. Residual gap: the container package is still private, so an anonymous pull requires changing its visibility to public. |
| 4. Certified configured Hermes capabilities and Runs behavior | Implemented with residual live probes | Configured contract tests cover authenticated discovery, exact once/deny approval, cancellation ordering, duplicate/reordered events, lost-stream recovery, `unknown_outcome`, restart/outage recovery without redispatch, discovery isolation, and task-scoped requests. The pinned combined runtime completed a Host-mediated synthetic task, and real external mutual-TLS discovery passed. Exact deny and cancel are now proven against the pinned runtime by `milestone1-macos-check`, which runs a native darwin Host against the pinned `0.21.1` gateway: a denied approval fails closed as `failed` with `approval_denied` and no dispatched run, and an explicit cancellation stops a genuinely in-flight run to a terminal `cancelled` with no output. Pinned-runtime lost-stream recovery and cross-machine external Runs probes remain open. |
| 5. Continuous formatting, vet, unit, race, contract, build, scanning, and deployment checks | Implemented for the tested Docker paths | `milestone1-linux-check` covers the combined journey. `milestone1-external-check` covers authenticated external adoption, rerun, doctor, Host-only lifecycle, and state preservation without claiming reboot or a real Hermes runtime. Both journeys passed twice clean on Azure. `milestone1-macos-check` covers the native darwin Host journey against the pinned Hermes gateway, so the pinned-runtime task and authorization contracts are exercised on a second platform. |
| 6. Interrupted setup, rerun, boot restart, update failure/rollback, task approval, cancellation, event-loss, and Host restart recovery | Partially implemented | Combined process interruption, rerun, real reboot, task completion, and state preservation pass. Same-VPS external separation, Host-only control, real reboot, and state preservation pass. A public `lumen update`/`lumen rollback` path now advances or restores one integrity-pinned artifact generation with atomic replacement, durable retention, and honest refusal of container-image generations; automated tests prove a failed replacement leaves the installed artifact byte-identical. Residual gaps: no real-machine update/rollback run, no container-image deployment update, and pinned-runtime lost-stream recovery remains open. |

The remaining real-runtime gaps keep the foundation exit open. Distribution is now real and verified, but the Hermes container package is still private, so an anonymous pull requires changing its visibility to public; same-VPS lifecycle separation is not cross-machine evidence; and automated tests for `lumen update`/`lumen rollback` are not a real-machine update and rollback proof. None may be inferred from synthetic tests, environment markers, templates, or command discovery.

### Azure owner evidence — 2026-09-14 IST / 2026-09-13 UTC

- Environment: Azure Ubuntu 22.04 amd64 (an owner-accepted variance from 24.04), Docker `29.1.3`, Compose `2.40.3`.
- Combined: the full gate passed twice clean using Hermes tag `v2026.9.7`, peeled commit `2237be355906fbe6065ce1815711eee52b2d646e`, archive SHA-256 `907c2a72db1c5dd637ea8eeae97f4cb5b32cef615c17258f6b190924ec5bf688`, and runtime `0.21.1`. An actual reboot changed boot ID from `ab67e507-8f3d-4661-a004-796f45eaaa3d` to `3bdc4b1a-31e9-43b9-a412-990fb2344f1c`; Host and Hermes container identities, state SHA-256 `4406069b0902a61660e4788ab19d12ac945003276b92f92ac7c89f62a9c931cc`, Space/Host/owner identity, epoch `1`, and the completed marker task were preserved. Public doctor returned `ready`. The first `/tmp`-backed reboot attempt failed honestly and led to the durable proof-root fix.
- External: a Host-only Compose project adopted independently supervised pinned Hermes `0.21.1` through bearer authentication, mutual TLS, and a pinned server certificate. Host restart and stop/start left Hermes container `efaac6...` and its TLS gateway `6e95ad...` running unchanged. An actual reboot changed boot ID from `3bdc4b1a-31e9-43b9-a412-990fb2344f1c` to `5747e142-55b0-4f5f-b1d5-4c9e49f45d9c`; Host `86ce65...`, Hermes, and gateway identities remained unchanged, state SHA-256 `55e850df70a5203363dbbd47fa7e1f8a31ec3b5b9c2eb800688564db19fbfbd4` was preserved, authenticated health returned `0.21.1`, and public doctor returned `ready`.
- Automated external: `scripts/lumen-external-check` passed twice clean with an independently supervised, ephemeral mutual-TLS compatibility endpoint. It proves public setup/rerun/doctor, Host-only lifecycle, and state preservation; it deliberately does not claim real Hermes behavior, reboot, or a second machine.
...
Initialization creates distinct recoverable owner and active Host identities. The active Host identity is the first local execution target and advertises the Hermes-backed capability. The loopback operator credential authenticates control requests and maps them to the owner principal, but it is not reused as an identity or encryption key. The create-once operation is crash-resumable before its marker: a later `init` strictly validates the complete encrypted bootstrap, reuses or recreates the operator credential, and durably commits the marker; partial or mismatched state fails closed without mutation.
