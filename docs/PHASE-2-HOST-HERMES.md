...
| 3. Reproducible integrity-pinned Lumen and Hermes distribution | Partially implemented | Hermes `v2026.9.7` resolves to peeled commit `2237be355906fbe6065ce1815711eee52b2d646e`; the official archive SHA-256 is pinned as `907c2a72db1c5dd637ea8eeae97f4cb5b32cef615c17258f6b190924ec5bf688`, and the frozen Docker build reports Hermes `0.21.1`. `scripts/lumen-release` builds `linux/amd64`, `linux/arm64`, `darwin/amd64`, `darwin/arm64`, and `android/arm64` twice, fails unless both builds are byte-identical, and emits the combined manifest with only measured sizes and digests. `scripts/install.sh` detects the host platform, resolves the artifact, and verifies size and SHA-256 before writing. Contract tests prove the emitted manifest loads in the product and that the installer refuses tampered bytes. `deploy/manifest-v1.json` remains the rejected fixture because the pinned Hermes image is not published yet: the registry push needs a token with `write:packages`, which the current login lacks. |
...
The remaining distribution and real-runtime gaps keep the foundation exit open. The checked-in manifest is still a rejected fixture; a loopback registry is not distributable; same-VPS lifecycle separation is not cross-machine evidence; and automated tests for `lumen update`/`lumen rollback` are not a real-machine update and rollback proof. None may be inferred from synthetic tests, environment markers, templates, or command discovery.
