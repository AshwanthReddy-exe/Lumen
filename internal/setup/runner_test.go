...
func TestRunnerResumesWithoutRepeatingCompletedStages(t *testing.T) {
	...
	if _, err := r.Run(context.Background(), Request{Topology: TopologyCombined, Profile: Development}); err == nil {
...
func TestRunnerRejectsInvalidRequestBeforeMutation(t *testing.T) {
	...
	report, err := r.Run(context.Background(), Request{Topology: TopologyCombined, Profile: Profile("invalid")})
...
func TestRunnerRejectsCompletedJournalForDifferentProfile(t *testing.T) {
	...
	report, err := r.Run(context.Background(), Request{Topology: TopologyCombined, Profile: Hardened})
...
func TestRunnerRejectsInitializerOnlyConfiguration(t *testing.T) {
	...
	if report, err := r.Run(context.Background(), Request{Topology: TopologyCombined, Profile: Development}); err == nil || report.Actions[0].Code != "setup_unavailable" {
		t.Fatalf("report=%#v err=%v", report, err)
	}
}
