...
func decodeFixture(data []byte, suite *FixtureSuite) error {
	...
	for _, tc := range suite.Cases {
		...
		for _, command := range tc.Commands {
			...
			switch command.Type {
			case CommandPairNode, CommandAdvertiseCapability, CommandSetGrant, CommandSubmit, CommandApprove, CommandComplete, CommandRecoverAfterRestart, CommandDispatchHostRun, CommandReconcileHostRun, CommandRequestHostRunCancellation, CommandCreateHostRun, CommandRequestRuntimeApproval, CommandResolveRuntimeApproval, CommandRecordRuntimeApproval:
				if command.HostID == "" {
