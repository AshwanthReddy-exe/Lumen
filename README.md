...
The complete product does **not** exist yet. Public setup, doctor, and service paths exist for combined and external deployments, but Milestone 1 release gates remain open. Canonical conversation records and a Host chat path are under review; production runtime certification, restart continuity, memory experience, web application, authenticated node transport, restricted file node, messaging continuity, Jarvis-like voice presence, managed service, and public SDK remain open. Platform examples such as macOS, Linux/VPS, Android Termux, and iPhone are evidence targets, not the roadmap's organizing principle.
